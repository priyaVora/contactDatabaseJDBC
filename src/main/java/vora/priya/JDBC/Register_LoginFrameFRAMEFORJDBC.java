...
public class Register_LoginFrameFRAMEFORJDBC extends GridPane {
	COntactDatabase_ForJDBC cd = null;
	public String appendingFileForCD;
	JDBC_CRUD jdbc;
	
	public Register_LoginFrameFRAMEFORJDBC(Stage primaryStage, COntactDatabase_ForJDBC cd, JDBC_CRUD jdbc) {
		this.jdbc = jdbc;
		this.cd = cd;
		start(primaryStage);
	}
...
//					LoginFrame frame = new LoginFrame(primaryStage);
					try {
						StorageTypeJDBC storageType = new StorageTypeJDBC(primaryStage, cd);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						e.printStackTrace();
					}
		...
		// #FFE4C4
		this.setStyle("-fx-background-color: #8B7740; " + "-fx-background-insets: 10; " + "-fx-background-radius: 20; "
